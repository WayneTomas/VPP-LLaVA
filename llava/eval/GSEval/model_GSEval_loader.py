...
import math
import copy
from llava.model.multimodal_encoder.detr.misc import NestedTensor
...
class CustomDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        qs = "Please provide the bounding box coordinate of the region this sentence describes: " + line["caption"] + extra_str
...
def eval_model(args):
    ...
    for (input_ids, image_tensor, image_sizes, extra_image, extra_mask), line in tqdm(zip(data_loader, questions), total=len(questions)):
        idx = line["idx"]
        cur_prompt = "Please provide the bounding box coordinate of the region this sentence describes: " + line["caption"]
        gt_bbox = line["box"]
        w, h = image_sizes[-1]
        ...
        ans_file.write(json.dumps({"idx": idx,
                                   "prompt": cur_prompt,
                                   "predicted_box": outputs,
                                   'gt_bbox': gt_bbox,
                                   'hw': [h, w],
                                   "model_id": model_name,
                                   }) + "\n")
