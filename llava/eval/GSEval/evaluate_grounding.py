...
class GroundingEvaluator:
    def __init__(self, image_dir: str = ".", gt_file: str = "GroundingSuite-Eval.jsonl", 
                 ...
                 mode: str = "box", model_type: str = "llava"):
        ...
        for idx, gt_item in enumerate(tqdm(self.gt_data)):
            ...
            if self.mode == "box":
                ...
                if self.model_type == "llava":
                    try:
                        pred_box = [float(num) for num in pred_box.strip('[]').split(', ')]
                    except:
                        pred_box = [0.0, 0.0, 0.0, 0.0]
                    pred_box = torch.tensor(pred_box)
                    h, w = pred_item.get("hw")
                    if h > w:
                        pred_box[0::2] *= h
                        pred_box[0::2] -= (h - w) // 2
                        pred_box[1::2] *= h
                        pred_box = F.relu(pred_box)
                    elif h < w:
                        pred_box[0::2] *= w
                        pred_box[1::2] *= w
                        pred_box[1::2] -= (w - h) // 2
...
def main():
    ...
    # add by wayne
    parser.add_argument('--model_type', type=str, default="llava", help='type of the MLLM')
