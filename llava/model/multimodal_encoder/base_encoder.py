...
class ProcessorWrapper:
    ...
    def preprocess(self, image, return_tensors='pt', **kwargs):
        # Ensure image is a PIL Image
        output = {}
        if type(self._transforms) is list:
            for t in self._transforms:
                image, target = t(image, **kwargs)
            output['image'] = image
            for k,v in target.items():
                output[k] = v       
        else:
            output['image'] = self._transforms(image, **kwargs)
        return output
