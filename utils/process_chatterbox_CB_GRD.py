...
def data_refine(data_list):
    ...
    for line in data_list:
        ...
        for i, conv in enumerate(conversations):
            if conv["from"] == "gpt":
                v = conv["value"]
                expression = v.split('.')[0]
                coordinates = v.split('.')[1]
                expression = expression.replace("I have provided the box of ", "") + '.'

                mod_question = f"<image>\nPlease provide the bounding box coordinate of the region this sentence describes: {expression}"
                match = coordinate_pattern.search(coordinates)
                if match:
                    ...
                    conversations[i-1]['value'] = mod_question
                    conversations[i]['value'] = f"{str(bbox_padded)}"
    ...
    # print("写入开始...")
    # with open("playground/data/Chatterbox_CB_GRD_refined.json", 'w', encoding='utf-8') as f:
    #     json.dump(new_data_list, f, ensure_ascii=False)
    

if __name__ == "__main__":
    data_path = "CB_GND.json"
